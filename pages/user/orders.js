...
export default function UserOrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await fetch("/api/orders/list?role=user");
  ...
  return (
    <UserLayout>
      ...
      <p className="text-gray-500 mb-4">Track your medicine orders</p>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-sm text-gray-500 mb-4">Once you select a shop, your order will appear here</p>
          <button
